...
class apriori:
  ...
  minSupportpercent: float
         minimum support percent of the frequent items(default 0.1)
  total_transactions: int
        total count of records in the data
  ...
  Methods
  -------
  Apriori(minSupportpercent,total_transactions, maximumBasketSize, min_confidence, vocab)
  ...
  generate_association_rules(results)
  ...
  def generate_association_rules(self, results, to_decode=False):
          """
          Method that generates the association rules which are support and confidence as a list. Return a df.
          ...
          Parameters
          ----------
          results: list
          ...
          #results without singletones
          #results = [item for item in results_decoded if type(item[0])!=str]
          sizes = [size for size in range(2,self.maximumBasketSize+1)]
          total_transactions = self.minSupport*10
